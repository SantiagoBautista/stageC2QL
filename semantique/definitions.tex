...
\begin{defi}
	Pour $f$ une fonction de $\val^2$ dans $\val$
	et $z$ un élément de $\val$,
	on appelle
	\emph{réduction d'une liste $l$ par la fonction $f$ avec l'élément neutre $z$}
	la valeur $\redu_{f, z}(l)$ définit par induction sur la liste par:
	$$
	\left\lbrace
	\begin{array}{l}
	\redu_{f,z}(\emptyset) = z \\
	\redu_{f,z}(l) = \redu_{f , f(z, hd(l))}(l)
	\end{array}
	\right.
	$$
\end{defi}
